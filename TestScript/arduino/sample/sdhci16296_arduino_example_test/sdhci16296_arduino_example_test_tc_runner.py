...
from tc.sdhci16296_arduino_example_test.sdhci16296_arduino_example_test_device import SDHCI16296TestDevice
...
def dut_file_monitor(test, source, data):
    global file_dict_index
    if '/mnt/sd0' in data:
        tmp = data.split()
        _file_dict[str(file_dict_index)] = tmp[1][9:]
        file_dict_index += 1
...
class SDHCITestTcRunner(TestRunner):
    def __init__(self, conf, **kwargs):
        ...
        self.sdhci16296_test_device = SDHCI16296TestDevice(
            kwargs.get('dut_device', None), 'SDHCI16296_TEST_DEVICE') if kwargs.get('dut_device', None)\
            else None
    ...
    def get_devices_list(self):
        return [self.sdhci16296_test_device]
    ...
    def build(self, debug=True, log=None, **kwargs):
        ...
        if not dut_bin:
            dut_bin = self.__build_binary(toolbox, self.defconfig_path, self.sdhci16296_test_device)
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        sdhci16296_test_device = self.get_devices_list()[0]

        setup = [
            Step(Action.EXECUTE, sdhci16296_test_device, ('dut_file_mon', dut_file_monitor),
                 Test.add_monitor),
        ]

        teardown = [
            Step(Action.EXECUTE, sdhci16296_test_device, 'dut_file_mon', Test.remove_monitor),
        ]

        Arduino_SDHCI_16296 = Test(
            name='Arduino_SDHCI_16296',
            timeout=timeout,
            setup=setup,
            test=[
                Step(Action.EXECUTE, sdhci16296_test_device, None,
                     lambda test, source, data: reset_data(test, source, data)),
                Step(Action.EXECUTE, sdhci16296_test_device, None,
                     lambda test, source, data: wait_sec(test, source, data, 1, 'wait')),
                Step(Action.EXECUTE, sdhci16296_test_device, None,
                     lambda test, source, data: checkDefaultDisk(test, source, data)),
                Step(Action.WAIT_FOR, sdhci16296_test_device, 'Start File Operation'),
                Step(Action.WAIT_FOR, sdhci16296_test_device, 'Finish File Operation'),
                Step(Action.WAIT_FOR, sdhci16296_test_device, 'Start USB Mass Storage Operation'),
                Step(Action.WAIT_FOR, sdhci16296_test_device, 'Finish USB MSC? (y/n)'),
                ...
                Step(Action.WAIT_FOR, sdhci16296_test_device, 'test done'),
            ],
            teardown=teardown
        )
        ...
        sdhci_gp = TestGroup(
            name='sdhci_gp',
            devices=[sdhci16296_test_device],
            tag=[Tag.POSITIVE],
            tests=[
                Arduino_SDHCI_16296,
            ]
