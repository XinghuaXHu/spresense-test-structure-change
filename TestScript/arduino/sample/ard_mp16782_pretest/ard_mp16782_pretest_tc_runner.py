...
from tc.ard_mp16782_pretest.ard_core_test_device import SpresenseTestDevice
...
class ArdMpTestTcRunner(TestRunner):
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        setup = [
            Step(Action.WAIT_FOR, spresense_test_device, 'Please input cmd'),
        ]

        teardown = [
        ]

        ARD_MP_Example_pretest_16782 = Test(
            name='ARD_MP_Example_pretest_16782',
            timeout=timeout,
            setup=setup,
            test=usb_s(spresense_test_device) + \
                 dsp_steps(spresense_test_device) + \
                 usb_e(spresense_test_device),
            teardown=teardown
        )
