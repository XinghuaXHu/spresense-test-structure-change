...
from tc.ard_mp16785_test.ard_core_test_device import SpresenseTestDevice
...
class ArdMpTestTcRunner(TestRunner):
    ...
    def __init__(self, conf, **kwargs):
        ...
        self.spresense_test_device = SpresenseTestDevice(
            kwargs.get('dut_device', None), 'MP16785_TEST_DEVICE') if kwargs.get('dut_device', None)\
            else None
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        ARD_MP_Example_test_16785 = Test(
            name='ARD_MP_Example_test_16785',
            timeout=timeout,
            setup=setup,
            test=[
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub1] Hello'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub2] Hello'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub3] Hello'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub4] Hello'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub5] Hello'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub1] Hello'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub2] Hello'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub3] Hello'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub4] Hello'),
