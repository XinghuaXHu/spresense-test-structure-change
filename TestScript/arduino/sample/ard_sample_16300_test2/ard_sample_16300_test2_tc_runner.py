...
from tc.ard_sample_16300_test2.ard_sample_16300_test2_device import MP3TestDevice
...
class ArdMp3TestTcRunner(TestRunner):
    ...
    def __init__(self, conf, **kwargs):
        ...
        self.mp3_test_device = MP3TestDevice(
            kwargs.get('dut_device', None), 'MP3_16300_TEST2_DEVICE') if kwargs.get('dut_device', None)\
            else None
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        ARD_Example_test_16300_MP3ENC_verify = Test(
            name='ARD_Example_test_16300_MP3ENC_verify',
            timeout=timeout,
            setup=setup,
            test=[
                Step(Action.EXECUTE, mp3_test_device, None,
                     lambda test, source, data: wait_sec(test, source, data, 5, 'wait for sd0')),
                Step(Action.WRITE, mp3_test_device, 'ls -l /mnt/spif/BIN/MP3ENC'),
                Step(Action.WAIT_FOR, mp3_test_device, '/mnt/spif/BIN/MP3ENC',
                     lambda test, source, data: check_file_exist(test, source, data)),
                Step(Action.WRITE, mp3_test_device, 'ls -l /mnt/sd0/BIN/MP3ENC'),
                Step(Action.WAIT_FOR, mp3_test_device, '/mnt/sd0/BIN/MP3ENC',
                     lambda test, source, data: check_file_exist(test, source, data)),
            ],
            teardown=teardown
        )

        ARD_Example_test_16300_MP3ENC_clear = Test(
            name='ARD_Example_test_16300_MP3ENC_clear',
            timeout=timeout,
            setup=setup,
            test=[
                Step(Action.EXECUTE, mp3_test_device, None,
                     lambda test, source, data: wait_sec(test, source, data, 5, 'wait for sd0')),
                Step(Action.WRITE, mp3_test_device, 'rm /mnt/spif/BIN/MP3ENC'),
                Step(Action.WRITE, mp3_test_device, 'rm /mnt/sd0/BIN/MP3ENC'),
                Step(Action.EXECUTE, mp3_test_device, None,
        ...
        def seleteTest(arguments):
            if arguments.clear == True:
                return [ARD_Example_test_16300_MP3ENC_clear, ]
            else:
                return [ARD_Example_test_16300_MP3ENC_verify, ]
