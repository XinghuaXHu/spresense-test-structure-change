...
from tc.ard_mp16784_test.ard_core_test_device import SpresenseTestDevice
...
def check_keyword(test, source, data):
    if 'Success' in data:
        return True
    else:
        test.set_fail(' '.join(['not find in ', data]))
    return False
...
class ArdMpTestTcRunner(TestRunner):
    ...
    def __init__(self, conf, **kwargs):
        ...
        self.spresense_test_device = SpresenseTestDevice(
            kwargs.get('dut_device', None), 'MP16784_TEST_DEVICE') if kwargs.get('dut_device', None)\
            else None
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        ARD_MP_Example_test_16784 = Test(
            name='ARD_MP_Example_test_16784',
            timeout=timeout,
            setup=setup,
            test=[
                Step(Action.WAIT_FOR, spresense_test_device, 'Send: id=100 data=0x'),
                Step(Action.WAIT_FOR, spresense_test_device, 'Recv: id=100 data=0x',
                     lambda test, source, data: check_keyword(test, source, data)),
                Step(Action.WAIT_FOR, spresense_test_device, 'Send: id=100 data=0x'),
                Step(Action.WAIT_FOR, spresense_test_device, 'Recv: id=100 data=0x',
                     lambda test, source, data: check_keyword(test, source, data)),
                Step(Action.WAIT_FOR, spresense_test_device, 'Send: id=100 data=0x'),
