...
from tc.ard_sample_16301_test1.ard_sample_16301_test1_device import MP3TestDevice
...
class ArdMp3TestTcRunner(TestRunner):
    ...
    def __init__(self, conf, **kwargs):
        ...
        self.mp3_test_device = MP3TestDevice(
            kwargs.get('dut_device', None), 'MP3_16301_TEST_DEVICE') if kwargs.get('dut_device', None)\
            else None
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        ARD_Example_test_16301_SRC_sd0 = Test(
            name='ARD_Example_test_16301_SRC_sd0',
            timeout=timeout,
            setup=setup,
            test=[
                Step(Action.WAIT_FOR, mp3_test_device, 'Select where to install SRC'),
                ...
                Step(Action.WAIT_FOR, mp3_test_device, 'Install: /mnt/sd0/BIN/SRC Done.'),
                Step(Action.WAIT_FOR, mp3_test_device, 'Finished.'),
            ],
            teardown=teardown
        )

        ARD_Example_test_16301_SRC_spif = Test(
            name='ARD_Example_test_16301_SRC_spif',
            timeout=timeout,
            setup=setup,
            test=[
                Step(Action.WAIT_FOR, mp3_test_device, 'Select where to install SRC'),
                ...
                Step(Action.WAIT_FOR, mp3_test_device, '2 -> SPI-Flash'),
                Step(Action.WAIT_FOR, mp3_test_device, 'Install: /mnt/spif/BIN/SRC Done'),
                Step(Action.WAIT_FOR, mp3_test_device, 'Finished.'),
            ],
            teardown=teardown
        )
        ...
        testGroup = TestGroup(
            ...
            tag=[Tag.POSITIVE],
            tests=[
                ARD_Example_test_16301_SRC_sd0,
                ARD_Example_test_16301_SRC_spif,
            ]
        )
