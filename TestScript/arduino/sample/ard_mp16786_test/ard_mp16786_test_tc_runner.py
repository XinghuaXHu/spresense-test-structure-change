...
from tc.ard_mp16786_test.ard_core_test_device import SpresenseTestDevice
...
class ArdMpTestTcRunner(TestRunner):
    ...
    def __init__(self, conf, **kwargs):
        ...
        self.spresense_test_device = SpresenseTestDevice(
            kwargs.get('dut_device', None), 'MP16786_TEST_DEVICE') if kwargs.get('dut_device', None)\
            else None
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        ARD_MP_Example_test_16786 = Test(
            name='ARD_MP_Example_test_16786',
            timeout=timeout,
            setup=setup,
            test=[
                Step(Action.WAIT_FOR, spresense_test_device, '[Main] Lock'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub1] Lock'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub2] Lock'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub3] Lock'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Main] Lock'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub1] Lock'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub2] Lock'),
                Step(Action.WAIT_FOR, spresense_test_device, '[Sub3] Lock'),
