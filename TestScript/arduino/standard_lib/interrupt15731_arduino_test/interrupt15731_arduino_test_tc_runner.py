...
from tc.interrupt15731_arduino_test.interrupt15731_arduino_test_device import Interrupt15731TestDevice
...
class InterruptTestTcRunner(TestRunner):
    def __init__(self, conf, **kwargs):
        ...
        self.interrupt15731_test_device = Interrupt15731TestDevice(
            kwargs.get('dut_device', None), 'INTERRUPT15731_TEST_DEVICE') if kwargs.get('dut_device', None)\
            else None
    ...
    def get_devices_list(self):
        return [self.interrupt15731_test_device]
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        interrupt15731_test_device = self.get_devices_list()[0]
        ...
        def verify_pin(device):
            ...
            for i_pin in range(len(PINS)):
                ...
                step += [
                    ...
                    Step(Action.WAIT_FOR, device, '------Start Test pin[' + str(PINS[i_pin]) + '] Mode CHANGE------'),
        ...
        arduino_interrupt_15731 = Test(
            name='arduino_interrupt_15731',
            timeout=timeout,
            setup=setup,
            test=verify_pin(interrupt15731_test_device),
            teardown=teardown
        )

        interrupt_gp = TestGroup(
            name='interrupt_gp',
            devices=[interrupt15731_test_device],
            tag=[Tag.POSITIVE],
            tests=[
                arduino_interrupt_15731,
            ]
        )
