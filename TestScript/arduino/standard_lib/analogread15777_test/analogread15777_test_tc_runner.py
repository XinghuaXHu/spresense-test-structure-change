...
from tc.analogread15777_test.analogread15777_test_device import AnalogReadTestDevice
...
def dut_analogread_monitor_2p5V(test, source, data):
    global value_count
    if 'voltage' in data:
        read_value = int(data.split("=")[1].strip(" "))
        if 440 <= read_value <= 513:
            test.log.debug('{} 2.5V ok'.format(data[0:2]))
        else:
            test.set_fail(' '.join(['Out of range. data = ', data]))
    if value_count > 100:
        test.add_user_event(source, 'Test Done')
...
class AnalogReadTestTcRunner(TestRunner):
    def __init__(self, conf, **kwargs):
        ...
        self.analogread15777_test_device = AnalogReadTestDevice(
    ...
    def get_devices_list(self):
        return [self.analogread15777_test_device]
    ...
    def build(self, debug=True, log=None, **kwargs):
        ...
        if not dut_bin:
            dut_bin = self.__build_binary(toolbox, self.defconfig_path, self.analogread15777_test_device)
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        teardown = [
            Step(Action.EXECUTE, analogread15777_test_device, None,
        ...
        ADN_Analog_io_15777 = Test(
            ...
            test=[
                Step(Action.EXECUTE, analogread15777_test_device, None,
                     lambda test, source, data: call_shell(test, source, data, "./kikusui_0V.sh")),
                Step(Action.WAIT_FOR, analogread15777_test_device, 'wait sec completed'),
                Step(Action.EXECUTE, analogread15777_test_device,
                     ('dut_analogread_monitor0', dut_analogread_monitor_0V), Test.add_monitor),
                Step(Action.WAIT_FOR, analogread15777_test_device, 'Test Done'),
                Step(Action.EXECUTE, analogread15777_test_device, 'dut_analogread_monitor0', Test.remove_monitor),

                Step(Action.EXECUTE, analogread15777_test_device, None,
                     lambda test, source, data: call_shell(test, source, data, "./kikusui_2.5V.sh")),
                Step(Action.WAIT_FOR, analogread15777_test_device, 'wait sec completed'),
                Step(Action.EXECUTE, analogread15777_test_device,
                     ('dut_analogread_monitor2p5', dut_analogread_monitor_2p5V), Test.add_monitor),
                Step(Action.WAIT_FOR, analogread15777_test_device, 'Test Done'),
                Step(Action.EXECUTE, analogread15777_test_device, 'dut_analogread_monitor2p5', Test.remove_monitor),
                ...
                Step(Action.WAIT_FOR, analogread15777_test_device, 'Test Done'),
                Step(Action.EXECUTE, analogread15777_test_device, 'dut_analogread_monitor5', Test.remove_monitor),
            ],
            teardown=teardown
        )
