...
from tc.ard_subcore16776_test.ard_core_test_device import SpresenseTestDevice
...
class ArdSubcoreTestTcRunner(TestRunner):
    ...
    def __init__(self, conf, **kwargs):
        ...
        self.spresense_test_device = SpresenseTestDevice(
            kwargs.get('dut_device', None), 'MP16776_TEST_DEVICE') if kwargs.get('dut_device', None)\
            else None
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        adn_subcore_softwareSerial_16776 = Test(
            name='adn_subcore_softwareSerial_16776',
            timeout=timeout,
            setup=setup,
            test=[
                ...
                Step(Action.WRITE, spresense_test_device, 'abcdddd11'),
                Step(Action.WAIT_FOR, spresense_test_device, 'mySoftSerial read =abcdddd11'),
                Step(Action.WAIT_FOR, spresense_test_device, 'mySoftSerial write=abcdddd11'),
                Step(Action.WRITE, spresense_test_device, '123r'),
                Step(Action.WAIT_FOR, spresense_test_device, 'mySoftSerial read =123r'),
                Step(Action.WAIT_FOR, spresense_test_device, 'mySoftSerial write=123r'),
            ],
            teardown=teardown
        )
        ...
        Spresense = TestGroup(
            ...
            tag=[Tag.POSITIVE],
            tests=[
                adn_subcore_softwareSerial_16776,
            ]
        )
