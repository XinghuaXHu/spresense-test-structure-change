...
from tc.ard_subcore16769_test.ard_core_test_device import SpresenseTestDevice
...
def dut_monitor(test, source, data):
    ...
    if 'TEMP' in data:
        ...
        test.log.debug('[' + str(count) + '] temp:' + temp + ' press:' + press + ' hum:' + hum)
...
class ArdMpTestTcRunner(TestRunner):
    ...
    def __init__(self, conf, **kwargs):
        ...
        self.spresense_test_device = SpresenseTestDevice(
            kwargs.get('dut_device', None), 'MP16769_TEST_DEVICE') if kwargs.get('dut_device', None)\
            else None
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        setup = [
            Step(Action.EXECUTE, spresense_test_device, ('dut_data_mon', dut_monitor),
                 Test.add_monitor),
        ]

        teardown = [
            Step(Action.EXECUTE, spresense_test_device, 'dut_data_mon', Test.remove_monitor),
        ]

        ARD_MP_Example_test_16769 = Test(
            name='ARD_MP_Example_test_16769',
            timeout=timeout,
            setup=setup,
            test=[
                ...
                Step(Action.WAIT_FOR, spresense_test_device, 'the task BME280 test ok!'),
            ],
            teardown=teardown
        )
