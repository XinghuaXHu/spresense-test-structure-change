...
from tc.ard_subcore16763_test.ard_core_test_device import SpresenseTestDevice
...
def dut_reboot_monitor(test, source, data):
    global value0
    global value1
    if '[Main] Maincore Value=0' in data:
        value0 += 1
    elif '[Main] Maincore Value=1' in data:
        value1 += 1
    elif '[Main] Maincore Value=' in data:
        test.set_fail(' '.join(['Value is Out of range. data = ', data]))
...
class ArdMpTestTcRunner(TestRunner):
    ...
    def __init__(self, conf, **kwargs):
        ...
        self.spresense_test_device = SpresenseTestDevice(
            kwargs.get('dut_device', None), 'MP16763_TEST_DEVICE') if kwargs.get('dut_device', None)\
            else None
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        ARD_MP_Example_test_16763 = Test(
            name='ARD_MP_Example_test_16763',
            timeout=timeout,
            setup=setup,
            test=[
