...
from tc.ard_subcore16764_test.ard_core_test_device import SpresenseTestDevice
...
def dut_reboot_monitor(test, source, data):
    ...
    if 'Voltage=' in data:
        index = data.find('=')
        vol_value = data[index+1:index+1+4]
        #vol_value = re.findall('[0-9]\.*',data)
        #vol_value = vol_value(0)
        try:
            vol_value = float(vol_value)
        except:
            test.log.debug('cant convert to float:'+data)
        else:
            if 3.6 >= vol_value >= 3.06:
                value0 += 1
            else:
...
class ArdMpTestTcRunner(TestRunner):
    ...
    def __init__(self, conf, **kwargs):
        ...
        self.spresense_test_device = SpresenseTestDevice(
            kwargs.get('dut_device', None), 'MP16764_TEST_DEVICE') if kwargs.get('dut_device', None)\
            else None
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        ARD_MP_Example_test_16764 = Test(
            name='ARD_MP_Example_test_16764',
            timeout=timeout,
            setup=setup,
            test=[
