...
from tc.audio_recorder_preproc_digmic.player_device import PlayerDevice
from tc.audio_recorder_preproc_digmic.recorder_device import RecorderDevice
...
PROJECT = 'spresense'
PLAYER_APPS = ['audio_player', 'msconn', 'msdis']
RECORDER_APPS = ['recorder']  # , 'msconn', 'msdis']
PLAYER_APPS_TO_BUILTIN = ['examples/audio_player', 'usbmsc']
RECORDER_APPS_TO_BUILTIN = ['recorder_preproc_digmic']

UPDATER = 'updater.py'
UPDATER_PATH = 'autotest/src/api'
DSP_PATH = 'sdk/modules/audio/dsp'
PREPROC_PATH = 'test/sqa/singlefunction/audio_recorder_preproc_digmic/worker'
DECODERS = ['MP3DEC', 'WAVDEC']
SRC = 'SRC'
ENCODERS = ['MP3ENC', 'PREPROC']

USBMSC_DEFCONFIG = 'defconfig/usbmsc-defconfig'
RECORDER_TEST_DEFCONFIG = 'defconfig/recorder_preproc_digmic-defconfig'
...
# noinspection PyUnusedLocal
def verify_recorded_file(test, source, data, filename, is_in_range=True, ch=8, dtype=16):
...
class AudioRecordTestTcRunner(TestRunner):
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        def usb_s(device, isbuiltin=True):
            r = umount(device) + [
                ...
                Step(Action.WRITE, device, 'msconn' if isbuiltin else 'recorder MSCONN', terminator='\n'),
        ...
        def usb_e(device, isbuiltin=True):
            r = [
                Step(Action.WRITE, device, 'msdis' if isbuiltin else 'recorder MSDIS'),
        ...
        tc_recorder_preproc_digmic_16728_0 = Test(
            name='tc_recorder_preproc_digmic_16728_0',
        ...
        tests = [tc_recorder_preproc_digmic_16728_0]
        for i in range(1, len(REC_FORMAT)):
            tests.append(Test(
                name='tc_recorder_preproc_digmic_16728_' + str(i),
                timeout=timeout,
        ...
        for i in range(0, len(REC_FORMAT)):
            tests.append(Test(
                name='tc_recorder_preproc_digmic_16730_' + str(i),
                timeout=timeout,
                setup=setup_steps(player_device, player_reboot_monitor, player_error_monitor)
                      + setup_steps(recorder_device, recorder_reboot_monitor, recorder_error_monitor)
                      + verify_sd_card_mount(player_device)
                ...
                [
                    ...
                    Step(Action.EXECUTE, recorder_device, None,
                         lambda test, source, data, index=i:
                         verify_recorded_file(test, source, data, gen_filename(test, 16730, index))),
                    Step(Action.WAIT_FOR, recorder_device, 'Recorded file verification finished'),
                ],
                teardown=teardown(player_device) + teardown(recorder_device)
            ))
