...
from tc.audio_pcm_capture.audio_test_device import AudioTestDevice
...
PROJECT = 'spresense'
AUIDO_APPS = ['audio_pcm_capture', 'zmodem']
APPS_TO_BUILTIN = ['examples/audio_pcm_capture','zmodem']
...
class AudioPcmCaptureTestTcRunner(TestRunner):

    def __init__(self, conf, **kwargs):
        super(AudioPcmCaptureTestTcRunner, self).__init__(conf)
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        SDK_Example_test_17161 = Test(
            name='SDK_Example_test_17161',
            timeout=timeout,
            setup=setup,
            test=[
                Step(Action.WRITE, audio_test_device, 'audio_pcm_capture'),
                Step(Action.WAIT_FOR, audio_test_device, 'Start PCM capture example'),
                ...
                Step(Action.WAIT_FOR, audio_test_device, 'Exit PCM capture example'),
            ],
            teardown=teardown
        )
...
if __name__ == "__main__":
    ...
    tc_runner = AudioPcmCaptureTestTcRunner(config, dut_device=dut_device)
