...
class AudioRecordTestTcRunner(TestRunner):
    ...
    def generate_test_groups(self, arguments, log=None):
            ...
            return [
                ...
                Step(Action.WAIT_FOR, device, 'Command cd / executed',
                     ...
                     execute_command(test, source, data, 'cd {}{}'.format(AUDIO_EXAMPLE_PATH, BIN),
                                     True)),
                Step(Action.WAIT_FOR, device, 'cd {}{}'.format(AUDIO_EXAMPLE_PATH, BIN),
                     lambda test, source, data: execute_command(test, source, data, 'rm PREPROC', False)),
            ]

        def generate_incorrect_preproc(device):
            return setup_steps(device, recorder_reboot_monitor) +\
                       verify_sd_card_mount(device) + [
                       Step(Action.WAIT_FOR, device, device.NSH_PROMPT),
                   ] + usb_s(device) + [
                       Step(Action.EXECUTE, device, None,
                            lambda test, source, data: cp_file(test, source, data, 'input/PREPROC', 'BIN')),
                       Step(Action.WAIT_FOR, device, 'copied'),
                   ] + usb_e(device)
        ...
        tc_recorder_preproc_error_16734 = Test(
            name='tc_recorder_preproc_error_16734',
            timeout=timeout,
            setup=recorder_setup(recorder_device, recorder_reboot_monitor) + rm_preproc(recorder_device),
            test=[
                Step(Action.WRITE, recorder_device, 'recorder LPCM 48K STEREO 16BIT EN'),
                Step(Action.WAIT_FOR, recorder_device, 'mptask_init() failure'),
                Step(Action.WAIT_FOR, recorder_device, 'subcode 18'),
            ],
            teardown=teardown(recorder_device)
        )

        tc_recorder_preproc_error_16735 = Test(
            name='tc_recorder_preproc_error_16735',
            timeout=timeout,
            setup=generate_incorrect_preproc(recorder_device),
            test=[
                Step(Action.WRITE, recorder_device, 'recorder LPCM 48K STEREO 16BIT EN'),
                Step(Action.WAIT_FOR, recorder_device, 'mptask_exec() failure'),
                Step(Action.WAIT_FOR, recorder_device, 'subcode 18'),
            ],
        ...
        audio = TestGroup(
            ...
            tag=[Tag.POSITIVE],
            tests=[tc_recorder_preproc_error_16734,
                   tc_recorder_preproc_error_16735],
        )
