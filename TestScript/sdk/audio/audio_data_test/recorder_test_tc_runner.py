...
class TestTcRunner(TestRunner):
    ...
    def __init__(self, conf, **kwargs):
        ...
        self.audio_test_device = TestDevice(kwargs.get('dut_device', None), 'AUDIO_RECORDER_TEST_DEVICE') \
            if kwargs.get('dut_device', None) else None

        if not all(self.get_devices_list()):
            raise TesterException('At least one device is needed!')

        self.project_path = '/examples/audio_recorder'
    ...
    def generate_test_groups(self, log=None):
        ...
        audio_tc_01 = Test(
            ...
            setup=setup,
            test=[
                Step(Action.WRITE, audio_test_device, TEST_DEVICE_APP_NAME),
                Step(Action.WAIT_FOR, audio_test_device, 'Exit AudioRecorder example'),
            ],
            teardown=teardown
        )
