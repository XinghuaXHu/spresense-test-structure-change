...
from tc.audio_recorder_digmic_wav_test.player_device import PlayerDevice
from tc.audio_recorder_digmic_wav_test.recorder_device import RecorderDevice
...
RECORDER_APPS = ['recorder_digmic_wav', 'zmodem']
PLAYER_APPS_TO_BUILTIN = ['examples/audio_player', 'zmodem']
RECORDER_APPS_TO_BUILTIN = ['recorder_digmic_wav_test']
...
RECORDER_TEST_DEFCONFIG = 'defconfig/recorder_digmic_wav_test-defconfig'
...
def execute_record(test, source, data, codec, sampling, channel, bitlen, confirm):
    ...
    command = '{} {} {} {} {}'.format('recorder_digmic_wav', codec, sampling, channel, bitlen)
...
# noinspection PyUnusedLocal
def verify_recorded_file(test, source, data, ch=1, dtype=16):
...
class AudioRecorderDigmicWavTestTcRunner(TestRunner):

    def __init__(self, conf, **kwargs):
        super(AudioRecorderDigmicWavTestTcRunner, self).__init__(conf)
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        tc_recorder_16334 = Test(
            name='tc_recorder_16334',
            ...
            test=[
                Step(Action.EXECUTE, player_device, None,
                     lambda test, source, data: sz_send_file(test, source, data, os.path.join(os.getcwd(), PLAYLIST_FILE))),
                Step(Action.WAIT_FOR, player_device,
                     'File {} successfully transferred'.format(PLAYLIST_FILE.split('/')[1]),
                     lambda test, source, data: mv_file(test, source, data,
                                                        PLAYLIST_FILE.split('/')[1], PLAYLIST)),
                Step(Action.WAIT_FOR, player_device, 'mv {} {}'.format(PLAYLIST_FILE.split('/')[1], PLAYLIST),
                     lambda test, source, data: execute_command(test, player_device, data, 'audio_player', False)),
                ...
                Step(Action.WAIT_FOR, recorder_device, 'successfully transferred',
                     lambda test, source, data: verify_recorded_file(test, source, data, 8, 16)),
...
if __name__ == "__main__":
    ...
    tc_runner = AudioRecorderDigmicWavTestTcRunner(config, player=player, recorder=recorder)
