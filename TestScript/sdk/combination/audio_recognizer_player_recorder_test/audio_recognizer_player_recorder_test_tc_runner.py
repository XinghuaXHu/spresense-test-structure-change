...
from tc.audio_recognizer_player_recorder_test.player_device import PlayerDevice
from tc.audio_recognizer_player_recorder_test.recorder_device import RecorderDevice
...
PROJECT = 'spresense'
PLAYER_APPS = ['recognizer_player', 'zmodem']
RECORDER_APPS = ['recognizer_recorder', 'zmodem']
PLAYER_APPS_TO_BUILTIN = ['recognizer_player', 'zmodem']
RECORDER_APPS_TO_BUILTIN = ['recognizer_recorder', 'zmodem']

UPDATER = 'updater.py'
UPDATER_PATH = 'autotest/src/api'
DSP_PATH = 'sdk/modules/audio/dsp'
RCGPROC_RECORDER_PATH = 'sqa/combination/recognizer_recorder/worker_recognizer'
RCGPROC_PLAYER_PATH = 'sqa/combination/recognizer_player/worker_recognizer'
DECODER = 'MP3DEC'
SRC = 'SRC'
RCGPROC = 'RCGPROC'

ZMODEM_DEFCONFIG = 'defconfig/zmodem-defconfig'
AUDIO_FILE = 'input/Sound.mp3'
RCGPROC_FILE = 'input/RCGPROC'
...
class AudioRecognizerPlayerAgingTestTcRunner(TestRunner):

    def __init__(self, conf, **kwargs):
        super(AudioRecognizerPlayerAgingTestTcRunner, self).__init__(conf)
        ...
        self.audio_dsp4 = os.path.join(os.getcwd(), RCGPROC_FILE)
    ...
    def generate_test_groups(self, arguments, log=None):
        ...
        def dsp_steps(device):
            r = [
                ...
                Step(Action.WAIT_FOR, device, 'copied'),
                Step(Action.EXECUTE, device, None,
                     lambda test, source, data: cp_file(test, source, data, self.audio_dsp4, 'BIN')),
                Step(Action.WAIT_FOR, device, 'copied'),
            ]
            return r
        ...
        tc_player_objif_17113_17114 = Test(
            name='tc_player_objif_17113_17114',
            ...
            test=usb_s(recorder_device) + \
                 ...
                 add_file_to_player() + \
                 usb_e(player_device) + [
                Step(Action.WRITE, player_device, 'recognizer_player'),
                Step(Action.WRITE, recorder_device, 'recognizer_recorder'),

                Step(Action.WAIT_FOR, player_device, 'Start Audio Recognizer'),
                Step(Action.WAIT_FOR, player_device, player_device.RUNNING_TIME),
                Step(Action.WAIT_FOR, recorder_device, 'Start AudioRecognizer example'),
                Step(Action.WAIT_FOR, recorder_device, player_device.RUNNING_TIME),
                Step(Action.WAIT_FOR, player_device, 'Exit AudioRecognizer'),
                Step(Action.WAIT_FOR, recorder_device, 'Exit AudioRecognizer example'),
...
if __name__ == "__main__":
    ...
    tc_runner = AudioRecognizerPlayerAgingTestTcRunner(config, player=player, recorder=recorder)
