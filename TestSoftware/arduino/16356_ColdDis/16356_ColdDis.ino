...
void pushed2(){ mySerial.println("Pushed D02!"); }
void pushed3(){ mySerial.println("Pushed D03!"); }
void pushed4(){ mySerial.println("Pushed D04!"); }
void pushed5(){ mySerial.println("Pushed D05!"); }
void pushed6(){ mySerial.println("Pushed D06!"); }
void pushed7(){ mySerial.println("Pushed D07!"); }
void pushed10(){ mySerial.println("Pushed D10!"); }
void pushed11(){ mySerial.println("Pushed D11!"); }
void pushed12(){ mySerial.println("Pushed D12!"); }
...
void disableBootCause(bootcause_e bc) {
  ...
  for (int j = 0; j < sizeof(pins)/sizeof(pins_s); j++) {
    pinMode(pins[j].pin, INPUT_PULLUP);
    attachInterrupt(pins[j].pin, pins[j].isr, FALLING);
  }
}
...
void setup()
{
  ...
  if ((bc == POR_SUPPLY) || (bc == POR_NORMAL)) {
    mySerial.println("spr_sdk-16356 start");
    EEPROM[0] = 0;
    EEPROM[1] = 0;
    EEPROM[2] = 0;
  ...
  mySerial.println(count%18);

  if (count < 9) {

    disableBootCause(COLD_RTC_ALM0);
    ...
    disableBootCause(COLD_PMIC_INT);
    ...
    if (pin_count < sizeof(pins) / sizeof(pins_s)) {
      disableBootCause(pins[pin_count].pin);
      mySerial.println("Goto cold sleep (GPIO)");
      mySerial.print("disabled pin = ");
      ...
      mySerial.print("main_count = ");
      mySerial.println(main_count);
      EEPROM[2] += 1;
      if (main_count < 10) {
        EEPROM[0] = 1;
        EEPROM[1] = 0;
        ...
        mySerial.println("Goto cold sleep (RTC)");
        mySerial.print("sleeptime = ");
        mySerial.println(sleeptimes[0]);
        delay(5000);
        LowPower.coldSleep(sleeptimes[0]);
        LowPower.end();
