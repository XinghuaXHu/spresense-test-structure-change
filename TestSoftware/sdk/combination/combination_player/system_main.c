...
const char * existBaseFileAddr_f = "/mnt/spif";
...
static int add_file(char * filename,int filesize,int filenum)
{
	...
	for(i=0;i<filenum;i++)
	{
		if(filename == NULL)
		{
			sprintf(fileaddr,"%s/%s_%d",existBaseFileAddr_f,"testfile",i);
		}
		else
		{
			sprintf(fileaddr,"%s/%s",existBaseFileAddr_f,filename);
		}
...
static void system_flash_task(void)
{
  while (1)
    {
      /* from start playing to stop playing */
      system_flash_main();
      sleep(1);
    }
...
int system_flash()
{
  ...
  ret = task_create("system_flash_task",
                    150,     /* task priority   */
                    2048,    /* task stack size */
                    (main_t)system_flash_task,
                    NULL);

  if (ret < 0)
    {
      printf("Failed to create task for system_flash. ret=%d\n", ret);
      return ret;
    }
