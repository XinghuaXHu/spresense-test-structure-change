...
#if 0  // getpeername not support on NuttX
        len = sizeof(sa);
...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
	...
	int i = 0;
	for (i = 1; i < argc; i++) {
		if (0 == strncmp(argv[i], "-f", MIN(strlen(argv[i]), strlen("-f")))) {
			i++;
			if (i >= argc) {
				printf("err: invalid -f option\n");
				continue;
			}
			switch (argv[i][0]) {
				case 'b':
				...
				case 'g':
				case 'G':
					test->settings->unit_format = argv[i][0];
					continue;
				default:
					printf("err: invalid -f, --format option\n");
					continue;
			}
		}
		else if(0 == strncmp(argv[i], "-p", MIN(strlen(argv[i]), strlen("-p")))){
			i++;
			if (i >= argc) {
				printf("err: invalid -p option\n");
				continue;
			}
			test->server_port = atoi(argv[i]);
		}
		else if(0 == strncmp (argv[i], "-d", MIN(strlen(argv[i]), strlen("-d")))){
			test->debug=1;
			continue;
		}
		else if(0 == strncmp (argv[i], "-l", MIN(strlen(argv[i]), strlen("-l")))){

			i++;

			blksize = unit_atoi(argv[i]);
			client_flag = 1;
			continue;
		}
		else if (0 == strncmp(argv[i], "-i", MIN(strlen(argv[i]), strlen("-i")))){
			i++;
			/* XXX: could potentially want separate stat collection and reporting intervals,
            but just set them to be the same for now */
			test->stats_interval = test->reporter_interval = atof(argv[i]);
            if ((test->stats_interval < MIN_INTERVAL || test->stats_interval > MAX_INTERVAL) && test->stats_interval != 0) {
                printf("The given interval(%d) is smaller or bigger than the max/min interval %d/%d\n",
				test->stats_interval, MIN_INTERVAL, MAX_INTERVAL);
				i_errno = IEINTERVAL;
                continue;
            }
		}
		else if(0 == strncmp(argv[i], "-c", MIN(strlen(argv[i]), strlen("-c")))){
			if (test->role == 's') {
				printf("Configured as server already\n");
                i_errno = IESERVCLIENT;
                continue;
            }
			i++;
		...
		else if(0 == strncmp(argv[i], "-s", MIN(strlen(argv[i]), strlen("-s")))){
			if (test->role == 'c') {
				printf("Configured as client already\n");
                i_errno = IESERVCLIENT;
                continue;
            }
			iperf_set_test_role(test, 's');
		...
		else if (0 == strncmp(argv[i], "-t", MIN(strlen(argv[i]), strlen("-t")))){
		...
		else  if (0 == strncmp(argv[i], "-w", MIN(strlen(argv[i]), strlen("-w")))){
		...
		else  if (0 == strncmp(argv[i], "-u", MIN(strlen(argv[i]), strlen("-u")))){

            set_protocol(test, Pudp);
			client_flag = 1;
			continue;
		}
		else  if (0 == strncmp(argv[i], "-b", MIN(strlen(argv[i]), strlen("-b")))){
            i++;

			test->settings->rate = (uint64_t)unit_atof_rate(argv[i]);
			rate_flag = 1;
			client_flag = 1;
			continue;
		}
		else  if (0 == strncmp(argv[i], "-R", MIN(strlen(argv[i]), strlen("-R")))){

			iperf_set_test_reverse(test, 1);
			client_flag = 1;
			continue;
		}
		else  if (0 == strncmp(argv[i], "--pacing-timer", MIN(strlen(argv[i]), strlen("--pacing-timer")))){
            i++;

			test->settings->pacing_timer = unit_atoi(argv[i]);
			client_flag = 1;
			continue;
		}
	}
...
iperf_new_stream(struct iperf_test *test, int s)
{
	...
	if (sp->diskfile_fd == -1) {
		printf("Failed to open new stream: %s\n", test->diskfile_name);
	    i_errno = IEFILE;
            munmap(sp->buffer, sp->test->settings->blksize);
            free(sp->result);
            free(sp);
    ...
    if ((readentropy(sp->buffer, test->settings->blksize) < 0) ||
		...
		printf("iperf_init_stream failed or entropy is negative\n");
        close(sp->buffer_fd);
        munmap(sp->buffer, sp->test->settings->blksize);
        free(sp->result);
        free(sp);
...
#if 0  // workaround getpeername not support on NuttX
    len = sizeof(struct sockaddr_storage);
...
#else
            printf("setsockopt(IPPROTO_IP, IP_TOS) not support\n");
            return -1;
#endif
        }
    }
