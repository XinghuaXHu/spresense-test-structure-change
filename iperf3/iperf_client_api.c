...
iperf_create_streams(struct iperf_test *test)
{
	...
	if (s > test->max_fd) test->max_fd = s;
        ...
        if (!sp) {
			printf("iperf_new_stream returned error\n");
            return -1;
		} else {
			printf("iperf_new_stream returned success\n");
		}
...
iperf_handle_message_client(struct iperf_test *test)
{
    ...
    switch (test->state) {
        ...
        case CREATE_STREAMS:
			...
			{
				printf("iperf_create_streams returned < 0\n");
				return -1;
			}
            break;
        case TEST_START:
...
iperf_connect(struct iperf_test *test)
{
	...
	test->ctrl_sck_mss = 1460;
	printf("Set test->ctrl_sck_mss: %d\n", test->ctrl_sck_mss);
...
{
    printf("Entering iperf_run_client\n");
	...
	if (result > 0) {
	    if (FD_ISSET(test->ctrl_sck, &read_set)) {
 	        if (iperf_handle_message_client(test) < 0) {
                    printf("Failed in iperf_handle_message_client\n");
		    return -1;
		}
